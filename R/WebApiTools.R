...
#' 
#' @details 
#' Load a Circe definition from a WebApi instance and insert it into this package. This will 
#' fetch the json object and store it in the 'inst/circe' folder, and fetch the template SQL
#' and store it in the 'inst/sql/sql_server' folder. Both folders will be created if they 
#' don't exist.
#' 
#' @param defintionId      The number indicating which Circe definition to fetch.
...
insertCirceDefinitionInPackage <- function(definitionId, 
  ...
  parsedJson <- rjson::fromJSON(json)
  if (is.null(name)){
  ...
  parsedExpression <- rjson::fromJSON(parsedJson$expression)
  jsonBody <- rjson::toJSON(list(expression = parsedExpression))
  ...
  cohortSqlJson <- RCurl::postForm(url, 
                                   .opts = list(httpheader = httpheader, 
                                                postfields = jsonBody))
  sql <- rjson::fromJSON(cohortSqlJson)$templateSql
