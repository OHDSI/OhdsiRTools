...
createArgFunction <- function(functionName,
  ...
  rCode <- c(rCode, "  }")
  # rCode <- c(rCode, paste(' attr(analysis, \'function\') <- \'', functionName, '\'', sep = ''))
  # rCode <- c(rCode, ' attr(analysis, \'userSpecified\') <- names(values)') rCode <- c(rCode,
  # 'packages <- installed.packages()') rCode <- c(rCode, 'attr(values, \'version\') <-
  # packages[packages[,\'Package\'] == \'CohortMethod\',\'Version\']')
