---
title: "Parallel execution using OhdsiRTools"
...
output:
  ...
  html_document:
    ...
    toc: yes
vignette: >
  %\VignetteIndexEntry{Parallel execution using OhdsiRTools}
...
This vignette describes how you can use the `OhdsiRTools` package to execute R code in parallel. This can help speed up an analysis, by using multiple processors at the same time instead of using a single processor to  execute the code in sequence. 
...
In this package, all nodes are on the same computer. Note that the `snow` package allows you to also create nodes on remote machines, for example in a physical computer cluster.
...
The *Logging using OhdsiRTools* vignette explains how logging can be used to record events, including warnings and errors, when executing in parallel.
...
# Support for ff objects

The `ff` package allows the user to work with data objects that are too large to fit in memory, by performing memory mapping to a file on disc. When calling `makeCluster`, the `fftempdir` option is copied from the main thread to all worker nodes, Also, the `ffmaxbytes` and `ffbatchbytes` options are set to divide the available memory across the nodes.
