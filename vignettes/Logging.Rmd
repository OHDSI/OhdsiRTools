---
title: "Logging using OhdsiRTools"
...
output:
  ...
  html_document:
    ...
    toc: yes
vignette: >
  %\VignetteIndexEntry{Logging using OhdsiRTools}
...
This vignette describes how you can use the `OhdsiRTools` package to perform logging. Logging is the activity of recording events that occur during an analysis in a log. The log can be used for example for for debugging, profiling (understanding performance bottlenecks), and audits.
...
```{r}
writeLines(readChar("log.txt", file.info("log.txt")$size))
```
...
```{r}
unlink("log.txt") # Clean up log file from the previous example
...
fun <- function(x) {
  OhdsiRTools::logInfo("The value of x is ", x)
  # Do something
  if (x == 6)
    OhdsiRTools::logDebug("X equals 6")
  return(NULL)
}
